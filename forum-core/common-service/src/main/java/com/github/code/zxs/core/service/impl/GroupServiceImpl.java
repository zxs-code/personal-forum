...
public class GroupServiceImpl extends ServiceImpl<GroupMapper, BaseGroup> implements GroupService {
    ...
    private List<TreeGroupDTO> getTreeGroup(List<BaseGroup> groups) {
        ...
        Map<Integer, List<TreeGroupDTO>> map = result.stream().collect(Collectors.groupingBy(BaseGroup::getParentId));
        result.forEach(item -> item.setChildren(map.get(item.getId())));
        return result;
    }
